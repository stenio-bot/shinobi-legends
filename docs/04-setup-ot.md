...
- **Morte de monstro em scripts de QA**: `g_map.getCreatureById(id)` continua devolvendo o monstro depois que ele morre (o índice por id só é limpo no reciclo de slot do protocolo), então `getHealthPercent()` fica preso em 0%. Detecte morte por `creature:getStackPos() == -1` ou pela mensagem de loot (ver `client-otc/tests/boss_recheck_rc.lua`).
